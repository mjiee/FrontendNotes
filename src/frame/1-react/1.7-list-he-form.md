...
对于文件表单&ltinput type=file&gt, 它的值只能由用户设置, 组件需要使用ref从DOM节点获取表单数据, 这种表单元素叫"非受控组件"。

```tsx
// 函数组件中的refs
function FileForm() {
    const file = React.useRef<HTMLInputElement>(null);  // 创建一个ref, 并指定初始值
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
