...
// 组合reducer
const reducer = combineReducers({ count: countReducer, user: userReducer.reducer });

// configureStore配置并创建store
const store = configureStore({
    reducer: reducer,
    middleware: [thunk, logger],  // 中间件, 可选
...
// 使用
function CountShow() {
    const value = useSelector<typeof store.getState, any>((state) => state);  // 传入参数类型和返回值类型
    ...
    return (
        <div>
            ...
            <button onClick={() => dispatch(userReducer.actions.setName("wang"))}>name</button>
            <button onClick={() => dispatch(userReducer.actions.addAge())}>age</button>
        </div>
    );
}
